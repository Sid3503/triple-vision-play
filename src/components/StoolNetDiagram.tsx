...
function FlowPulse({
  ...
  return (
    <>
      ...
      <circle ref={dotRef} r={5} fill={color} style={{ filter: `drop-shadow(0 0 8px ${color})` }} />
    </>
  );
}
      ...
      <div className="px-6 pt-0">
        <div className="mx-auto max-w-[1280px]">
          ...
          <header className="mb-8 flex flex-wrap items-end justify-between gap-6 border-b border-border pb-6">
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm text-ink-soft">
                A multi-task deep network for Bristol Stool Scale (BSS) classification — joining a
                pretrained backbone with three parallel attention mechanisms feeding three
                ...
                onMouseEnter={(e) =>
                  showTip(
                    ...
                    <span className="block max-w-[200px] leading-snug">
                      Open the equations panel — each formula includes a plain-English explanation
                      and variable guide.
                    </span>,
                  )
                }
                        ...
                        <motion.div
                          ...
                          onMouseEnter={(e) => {
                            ...
                            showTip(
                              e,
                              <>
                                <strong className="font-display text-white">{b.title}</strong>
                                <span className="mt-1 block text-[#cfcfd6]">{b.tip}</span>
                                <span className="mt-1 block text-white">{b.formula}</span>
                              </>,
                            );
                          }}
                          onMouseMove={moveTip}
                        ...
                        >
                          ...
                          <ol className="flex flex-col gap-1.5">
                            {b.steps.map((s, j) => (
                              ...
                              >
                                <span className="text-ink-mute">{j + 1} ·</span> {s}
                              </motion.li>
                            ))}
                          </ol>
                          <div
              ...
              <div className="grid w-full max-w-[1000px] grid-cols-1 gap-4 md:grid-cols-3">
                {HEADS.map((h, i) => (
                  <motion.div
                    ...
                    onMouseEnter={(e) =>
                      showTip(
                        e,
                        <>
                          <strong className="font-display text-white">{h.title}</strong>
                  ...
                  >
                    ...
                    />
                    <h4 className="mt-1 font-display text-sm font-bold">{h.title}</h4>
                    <div className="mb-1.5 font-mono text-xs text-ink-soft">{h.arch}</div>
                    <span className="inline-block rounded-full bg-surface-soft px-2 py-0.5 font-mono text-[11px]">
                      {h.lossShort}
                    </span>
                    <div className="mt-2 font-mono text-[10.5px] text-ink-soft">{h.classes}</div>
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <footer className="mt-10 flex flex-wrap items-center gap-4 border-t border-border pt-5 font-mono text-[11px] text-ink-soft">
            <Swatch
              c="var(--pos)"
              l="Position Attention"
              tip="Spatial self-attention branch: each pixel attends to every other pixel."
              onEnter={showTip}
              onMove={moveTip}
              onLeave={hideTip}
            />
            <Swatch
              c="var(--chan)"
              l="Channel Attention"
              tip="Channel correlation branch: re-weights feature channels by co-activation."
              onEnter={showTip}
              onMove={moveTip}
              onLeave={hideTip}
            />
            <Swatch
              c="var(--type)"
              l="Type Attention"
              tip="Multi-scale texture branch with sigmoid ordinal gate for BSS 1–7."
              onEnter={showTip}
              onMove={moveTip}
              onLeave={hideTip}
            />
            <Swatch
              c="var(--fusion)"
              l="Fusion"
              tip="Equal-weight average of all three attention outputs: F=(P+C+T)/3."
              onEnter={showTip}
              onMove={moveTip}
              onLeave={hideTip}
            />
            <Swatch
              c="var(--head-type)"
              l="Type Head"
              tip="Primary head (512→256→7). Focal Loss ×2.0 to handle class imbalance across BSS Types 1–7."
              onEnter={showTip}
              onMove={moveTip}
              onLeave={hideTip}
            />
            <Swatch
              c="var(--head-shape)"
              l="Shape Head"
              tip="Auxiliary head (256→4): Constipation / Normal / Mild Diarrhea / Liquid. CrossEntropy ×0.8."
              onEnter={showTip}
              onMove={moveTip}
              onLeave={hideTip}
            />
            <Swatch
              c="var(--head-color)"
              l="Color Head"
              tip="Auxiliary head (128→2): Brown / Yellow. Low-weight (CrossEntropy ×0.1) regulariser."
              onEnter={showTip}
              onMove={moveTip}
              onLeave={hideTip}
            />
            <span className="ml-auto">L = 2.0·L_type + 0.8·L_shape + 0.1·L_color</span>
          </footer>
...
const Node = ({
  ...
  const styles: Record<string, string> = {
    ...
    backbone: "bg-gradient-to-b from-card to-surface-soft border-foreground/15 min-w-[280px]",
    fusion: "border-fusion min-w-[260px]",
    gap: "bg-card border-border min-w-[220px]",
  };
  const fusionStyle = variant === "fusion" ? { background: "var(--fusion-soft)" } : {};
  ...
  return (
    <motion.div
      ...
      style={fusionStyle}
    >
      {title && <div className="font-display text-[13px] font-bold tracking-tight">{title}</div>}
...
const Swatch = ({
  ...
  <span
    ...
    onMouseEnter={
      tip && onEnter
        ? (e) => onEnter(e, <span className="block max-w-[220px] leading-snug">{tip}</span>)
        : undefined
    }
    onMouseMove={onMove}
        ...
        {REFERENCES.map((ref) => (
          ...
          >
            ...
            <div className="mt-1 flex items-center justify-between">
              <span className="font-accent text-[11px] font-bold" style={{ color: ref.color }}>
                {ref.index}
              </span>
              <span
            ...
            {/* Title */}
            <p
              className="font-display text-[13px] font-bold leading-snug group-hover:underline decoration-1 underline-offset-2"
            ...
            {/* Note */}
            <p
              className="mt-auto border-t border-dashed border-border pt-3 font-mono text-[10.5px] leading-relaxed"
...
const EqCard = ({
  ...
  >
    ...
    <div className="px-3.5 py-3">
      ...
      <div className="font-mono text-[9.5px]" style={{ color: "var(--ink-mute)" }}>
        tensor: {io}
      </div>
    </div>
  </motion.div>
